...
- Resource counts from different synthesis/technology-mapping stacks are not
  interchangeable hardware units. Convert capacity axes only with independent
  same-RTL probes whose structures resist mapper collapse. In an application
  holdout, gate the behaviorally relevant per-resource FPGA-count/capacity
  decisions; retain raw cross-mapper count error as a visible diagnostic and
  never tune or waive it using the holdout workload.
- Never label an academic mapper-equivalent capacity as a physical LUT/FF/
  BRAM/DSP count. Every calibrated model and emitted BoardDB must carry three
  explicit axes: public/authorized physical-device capacity, reference-flow
  reported capacity, and academic mapper-equivalent capacity. The BoardDB
  `capacity` field is the partitioner's academic axis, not a physical device
  datasheet count.
- A selectable specification must prove that its fitted reference capacity
  interval contains the declared capacity of exactly one named physical FPGA.
  Aggregate board, module, or multi-die observations must never be relabeled
  as one FPGA. If a reference configuration does not expose an independently
  identifiable single-FPGA target, keep it out of automatic family selection.
- A reduced-utilization stress BoardDB must be generated by an explicit policy
  override that can only lower the model's declared utilization limit. Encode
  that override in the platform identity and never report it as the default
...
- Production Phase 6 performs a bounded one-macro-cycle smoke replay for each
  ...
  replay the implicit cost of every Phase 1--7 run.
