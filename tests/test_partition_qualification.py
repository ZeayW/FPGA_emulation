...
class PartitionQualificationTest(unittest.TestCase):
    ...
    def test_source_commit_and_direct_symlink_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            config_path = self._config(root)
            config = json.loads(config_path.read_text())
            ...
            config_path.write_text(json.dumps(config), encoding="utf-8")
            with self.assertRaisesRegex(ValidationError, "repository HEAD"):
                compile_partition_qualification_spec(
                    config_path, REPOSITORY, root / "wrong-commit.json"
                )

            config["source_commit"] = _repository_commit()
            linked_rtl = root / "linked-rtl.v"
            linked_rtl.symlink_to(Path(config["rtl_source"]))
            config["rtl_source"] = str(linked_rtl)
            config_path.write_text(json.dumps(config), encoding="utf-8")
            with self.assertRaisesRegex(ValidationError, "symbolic link"):
                compile_partition_qualification_spec(
                    config_path, REPOSITORY, root / "symlink.json"
                )

    def test_tool_bytes_change_partition_input_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            config_path = self._config(root)
            first_output = root / "first.json"
            compile_partition_qualification_spec(
                config_path, REPOSITORY, first_output
            )
            first = json.loads(first_output.read_text())

            config = json.loads(config_path.read_text())
            refiner = Path(config["tools"]["mfspart_refiner"])
            refiner.write_text("changed refiner bytes\n", encoding="utf-8")
            second_output = root / "second.json"
            compile_partition_qualification_spec(
                config_path, REPOSITORY, second_output
            )
            second = json.loads(second_output.read_text())
            first_partition = first["nodes"][2]
            second_partition = second["nodes"][2]
            self.assertNotEqual(
                first_partition["inputs"]["tool.mfspart_refiner"],
                second_partition["inputs"]["tool.mfspart_refiner"],
            )
            self.assertEqual(
                first_partition["command_identity"],
                second_partition["command_identity"],
            )
