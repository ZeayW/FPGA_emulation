        ...
        try:
            with gzip.open(filename, "rb") as f:
                ...
                if self.data_cls.inst_sizes.is_cuda:
                    ...
                    self.op_cls.density_op.reset()
                    self.op_cls.overflow_op.reset()
                # A serialized density operator may predate the separate live
                # optimization mask or retain a tensor owned by the producer
                # process/device.  Rebind it to this placement's current mask
                # so subsequent legalization updates remain observable.
                self.op_cls.density_op.area_type_mask = (
                    self.data_cls.optimization_area_type_mask
                )
