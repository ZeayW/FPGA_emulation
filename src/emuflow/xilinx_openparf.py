...
def export_xilinx_cluster_bookshelf(
    ...
    *,
    ...
    config = {
        ...
        # A packed slice is one legal site, but its external pin demand and
        # the RUDY generated by its nets can differ by more than an order of
        # magnitude from another packed slice.  A pure one-node/one-site
        # density solve therefore creates legal yet severely unroutable
        # guidance.  Retain the exact one-site base area and let OpenPARF's
